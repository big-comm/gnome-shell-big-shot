...
export class ZoomCalloutAction extends DrawingAction {
    ...
    // P / M / G modifiers around the toolbar's selected text size.
    static get CAPTION_SCALES() { return [0.75, 1, 1.5]; }
    ...
    /** Caption font size in image space, based on the toolbar size. */
    captionFontSize() {
        const scale = ZoomCalloutAction.CAPTION_SCALES[this.captionSizeIndex] ?? 1;
        return Math.max(8, Math.min(200, this.options.size * 5 * scale));
    }
    ...
    generatePreview(pixbuf, bufScale) {
        ...
        try {
            tmpDir = GLib.dir_make_tmp('big-shot-zoom-XXXXXX');
            tmpPath = GLib.build_filenamev([tmpDir, 'preview.png']);
